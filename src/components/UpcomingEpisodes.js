...
import { Cpu, ShoppingBag, BookOpen, Users, Workflow, Globe } from 'lucide-react';
...
const EPISODES = [
  ...
  {
    id: 'ep3',
    title: 'EP 04 — TRUST & TRANSPARENCY',
...
export default function UpcomingEpisodes() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setActiveState((prev) => ({
        active: (prev.active + 1) % EPISODES.length,
        prevActive: prev.active
      }));
    }, 6400); // Doubled wait time to 6.4s between animations
